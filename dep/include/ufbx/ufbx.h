...
#if defined(_MSC_VER)
#pragma warning(push)
#pragma warning(disable : 4061) // enumerator 'ENUM' in switch of enum 'enum' is
                                // not explicitly handled by a case label
#pragma warning(                                                               \
    disable : 4201) // nonstandard extension used: nameless struct/union
#pragma warning(disable : 4505) // unreferenced local function has been removed
#pragma warning(disable : 4820) // type': 'N' bytes padding added after data
                                // member 'member'
...
template <typename T, typename U> struct ufbxi_type_is {};

template <typename T> struct ufbxi_type_is<T, T> {
  using type = int;
};

template <typename T> struct ufbx_converter {};

#define UFBX_CONVERSION_IMPL(p_name)                                           \
  template <typename T,                                                        \
            typename S =                                                       \
                typename ufbxi_type_is<T, decltype(ufbx_converter<T>::from(*(  \
                                              const p_name *)nullptr))>::type> \
  operator T() const {                                                         \
    return ufbx_converter<T>::from(*this);                                     \
  }

#define UFBX_CONVERSION_TO_IMPL(p_name)                                        \
  template <typename T, typename S = typename ufbxi_type_is<                   \
                            p_name, decltype(ufbx_converter<T>::to(            \
                                        *(const T *)nullptr))>::type>          \
  p_name(const T &t) {                                                         \
    *this = ufbx_converter<T>::to(t);                                          \
  }

#define UFBX_CONVERSION_LIST_IMPL(p_name)                                      \
  template <typename T, typename S = typename ufbxi_type_is<                   \
                            T, decltype(ufbx_converter<T>::from_list(          \
                                   (p_name *)nullptr, (size_t)0))>::type>      \
  operator T() const {                                                         \
    return ufbx_converter<T>::from_list(data, count);                          \
  }
...
#define UFBX_VERTEX_ATTRIB_IMPL(p_type)                                        \
  p_type &operator[](size_t index) const {                                     \
    ufbx_assert(index < indices.count);                                        \
...
#define UFBX_CALLBACK_IMPL(p_name, p_fn, p_return, p_params, p_args)           \
  template <typename F> static p_return _cpp_adapter p_params {                \
    F &f = *static_cast<F *>(user);                                            \
    return f p_args;                                                           \
  }                                                                            \
  p_name() = default;                                                          \
  p_name(p_fn *f) : fn(f), user(nullptr) {}                                    \
  template <typename F>                                                        \
  p_name(F *f) : fn(&_cpp_adapter<F>), user(static_cast<void *>(f)) {}
#else
#define UFBX_CALLBACK_IMPL(p_name, p_fn, p_return, p_params, p_args)
#endif
...
#define ufbx_pack_version(major, minor, patch)                                 \
  ((uint32_t)(major) * 1000000u + (uint32_t)(minor) * 1000u + (uint32_t)(patch))
...
UFBX_ENUM_TYPE(ufbx_rotation_order, UFBX_ROTATION_ORDER,
               UFBX_ROTATION_ORDER_SPHERIC);
...
UFBX_ENUM_TYPE(ufbx_dom_value_type, UFBX_DOM_VALUE_TYPE,
               UFBX_DOM_VALUE_ARRAY_IGNORED);
...
// eg. `prop->value_real` and `prop->value_int` have the same value (well,
// close) if `prop->type == UFBX_PROP_INTEGER`. String values are not converted
// from/to.
...
typedef enum ufbx_prop_flags UFBX_FLAG_REPR {
  ...
  // where
  // `ufbx_connection.dst_prop` is this property and `ufbx_connection.src_prop`
  // is defined.
  ...
  // Value type.
  // `REAL/VEC2/VEC3/VEC4` are mutually exclusive but may coexist with eg.
  // `STRING`
...
UFBX_ENUM_TYPE(ufbx_element_type, UFBX_ELEMENT_TYPE,
...
// inherited correctly.
// NOTE: These don't map to `"InheritType"` property as there may be new ones
...
UFBX_ENUM_TYPE(ufbx_inherit_mode, UFBX_INHERIT_MODE,
               UFBX_INHERIT_MODE_COMPONENTWISE_SCALE);
...
struct ufbx_node {
  ...
  // Node where scale is inherited from for
  // `UFBX_INHERIT_MODE_COMPONENTWISE_SCALE` and even for
  // `UFBX_INHERIT_MODE_IGNORE_PARENT_SCALE`. For componentwise-scale nodes,
  // this will point to `parent`, for scale ignoring nodes this will point to
  // the parent of the nearest componentwise-scaled node in the parent chain.
  ...
  ufbx_vec3
      adjust_pre_translation; // < Translation applied between parent and self
  ...
  ufbx_quat
      adjust_post_rotation;    // < Rotation applied in local space at the end
  ...
  ufbx_mirror_axis
      adjust_mirror_axis; // < Mirror translation and rotation on this axis

  // Materials used by `mesh` or other `attrib`.
  // There may be multiple copies of a single `ufbx_mesh` with different
  // materials in the `ufbx_node` instances.
...
typedef struct ufbx_uv_set {
  ...
  ufbx_vertex_vec3
      vertex_tangent; // < (optional) Tangent vector in UV.x direction
  ufbx_vertex_vec3
      vertex_bitangent; // < (optional) Tangent vector in UV.y direction
} ufbx_uv_set;
...
UFBX_LIST_TYPE(ufbx_subdivision_weight_range_list,
               ufbx_subdivision_weight_range);
...
UFBX_ENUM_TYPE(ufbx_subdivision_display_mode, UFBX_SUBDIVISION_DISPLAY_MODE,
               UFBX_SUBDIVISION_DISPLAY_SMOOTH);
...
//   A B D   B C E D   C E F
//   vertex_position.data[vertex_position.indices[index]]
//
...
//   ^ ^ ^ v v v
//   vertex_normal.data[vertex_normal.indices[vertex_first_index[vertex]]]
//
struct ufbx_mesh {
  union {
    ufbx_element element;
  ...
  // NOTE: May be zero in valid meshes if the file doesn't contain edge
  // adjacency data!
  size_t num_edges;

  size_t max_face_triangles; // < Maximum number of triangles in a  face in this
                             // mesh
  ...
  ufbx_uint32_list face_material; // < Indices to `ufbx_mesh.materials[]` and
                                  // `ufbx_node.materials[]`
  ufbx_uint32_list face_group;    // < Face polygon group index, indices to
                                  // `ufbx_mesh.face_groups[]`
  ...
  // First index referring to a given vertex, `UFBX_NO_INDEX` if the vertex is
  // unused.
  ufbx_uint32_list vertex_first_index;

  // Vertex attributes, see the comment over the struct.
  //
  // NOTE: Not all meshes have all attributes, in that case `indices/data ==
  // NULL`!
  //
  ...
  ufbx_vertex_vec3
      vertex_normal;          // < (optional) Normal vectors, always defined if
                              // `ufbx_load_opts.generate_missing_normals`
  ...
  ufbx_vertex_real
      vertex_crease; // < (optional) Crease value for subdivision surfaces
  ...
  // Use `ufbx_node.materials[]` to get the per-instance materials at the same
  // indices.
  ufbx_material_list materials;
  ...
  // Either from missing normals (via
  // `ufbx_load_opts.generate_missing_normals`), skinning, tessellation, or
  // subdivision.
...
typedef enum ufbx_light_type UFBX_ENUM_REPR {
  ...
  UFBX_LIGHT_POINT,
  // Infinite directional light pointing locally towards
  // `light->local_direction`
  // For global: `ufbx_transform_direction(&node->node_to_world,
  // light->local_direction)`
  UFBX_LIGHT_DIRECTIONAL,
  // Cone shaped light towards `light->local_direction`, between
  // `light->inner/outer_angle`.
  // For global: `ufbx_transform_direction(&node->node_to_world,
  // light->local_direction)`
  UFBX_LIGHT_SPOT,
...
UFBX_ENUM_TYPE(ufbx_light_area_shape, UFBX_LIGHT_AREA_SHAPE,
               UFBX_LIGHT_AREA_SHAPE_SPHERE);
...
struct ufbx_light {
  ...
  // Color and intensity of the light, usually you want to use `color *
  // intensity` NOTE: `intensity` is 0.01x of the property `"Intensity"` as that
  // matches matches values in DCC programs before exporting.
  ufbx_vec3 color;
  ufbx_real intensity;
...
UFBX_ENUM_TYPE(ufbx_projection_mode, UFBX_PROJECTION_MODE,
               UFBX_PROJECTION_MODE_ORTHOGRAPHIC);
...
UFBX_ENUM_TYPE(ufbx_aspect_mode, UFBX_ASPECT_MODE,
               UFBX_ASPECT_MODE_FIXED_HEIGHT);
...
typedef enum ufbx_aperture_mode UFBX_ENUM_REPR {
  // Use separate `"FieldOfViewX"` and `"FieldOfViewY"` as horizontal/vertical
  // FOV angles
  UFBX_APERTURE_MODE_HORIZONTAL_AND_VERTICAL,
  // Use `"FieldOfView"` as horizontal FOV angle, derive vertical angle via
  // aspect ratio
  UFBX_APERTURE_MODE_HORIZONTAL,
  // Use `"FieldOfView"` as vertical FOV angle, derive horizontal angle via
  // aspect ratio
...
UFBX_ENUM_TYPE(ufbx_aperture_mode, UFBX_APERTURE_MODE,
               UFBX_APERTURE_MODE_FOCAL_LENGTH);
...
typedef enum ufbx_gate_fit UFBX_ENUM_REPR {
  ...
  UFBX_GATE_FIT_NONE,
  // Fit the render gate to the height of the film, derive width from aspect
  // ratio
  UFBX_GATE_FIT_VERTICAL,
  // Fit the render gate to the width of the film, derive height from aspect
  // ratio
...
typedef enum ufbx_aperture_format UFBX_ENUM_REPR {
  ...
  UFBX_APERTURE_FORMAT_35MM_ANAMORPHIC,     // < 0.864 x 0.732 inches (squeeze
                                            // ratio: 2)
...
UFBX_ENUM_TYPE(ufbx_aperture_format, UFBX_APERTURE_FORMAT,
...
UFBX_ENUM_TYPE(ufbx_coordinate_axis, UFBX_COORDINATE_AXIS,
               UFBX_COORDINATE_AXIS_UNKNOWN);
...
struct ufbx_camera {
  ...
  // Equal to `field_of_view_tan` if perspective, `orthographic_size` if
  // orthographic.
  ...
  // FBX saves cameras with +X forward and +Y up, but you can override this
  // using `ufbx_load_opts.target_camera_axes` and it will be reflected here.
...
// Segment of a `ufbx_line_curve`, indices refer to
// `ufbx_line_curve.point_indices[]`
...
struct ufbx_line_curve {
  ...
  ufbx_vec3_list
      control_points; // < List of possible values the line passes through
  ufbx_uint32_list
      point_indices; // < Indices to `control_points[]` the line goes through
...
UFBX_ENUM_TYPE(ufbx_nurbs_topology, UFBX_NURBS_TOPOLOGY,
               UFBX_NURBS_TOPOLOGY_CLOSED);
...
struct ufbx_nurbs_curve {
  ...
  // NOTE: The control points are _not_ homogeneous, meaning you have to
  // multiply them by `w` before evaluating the surface.
...
typedef enum ufbx_lod_display UFBX_ENUM_REPR {
  UFBX_LOD_DISPLAY_USE_LOD, // < Display the LOD level if the distance is
                            // appropriate.
...
struct ufbx_lod_group {
  ...
  // If set to `true`, `ufbx_lod_level.distance` represents a screen size
  // percentage.
  ...
  // If set to `true` don't account for parent transform when computing the
  // distance.
  bool ignore_parent_transform;

  // If `use_distance_limit` is enabled hide the group if the distance is not
  // between `distance_limit_min` and `distance_limit_max`.
...
typedef enum ufbx_skinning_method UFBX_ENUM_REPR {
  ...
  // Blend between `UFBX_SKINNING_METHOD_LINEAR` and
  // `UFBX_SKINNING_METHOD_BLENDED_DQ_LINEAR`
  // The blend weight can be found either per-vertex in
...
UFBX_ENUM_TYPE(ufbx_skinning_method, UFBX_SKINNING_METHOD,
               UFBX_SKINNING_METHOD_BLENDED_DQ_LINEAR);
...
typedef struct ufbx_skin_vertex {
  ...
  // Should be used if `skinning_method ==
  // UFBX_SKINNING_METHOD_BLENDED_DQ_LINEAR`
  ufbx_real dq_weight;
...
// the binding matrix and a `ufbx_node *bone` that has the current
// transformation.
struct ufbx_skin_deformer {
  ...
  // Blend weights between Linear Blend Skinning (0.0) and Dual Quaternion
  // (1.0). HINT: You probably want to use `vertices` and
  // `ufbx_skin_vertex.dq_weight` instead! NOTE: These may be out-of-bounds for
  // a given mesh, `vertices` is always safe.
...
struct ufbx_skin_cluster {
  ...
  // ie. `ufbx_matrix_mul(&cluster->bone->node_to_world,
  ...
  // HINT: It may be simpler to use `ufbx_skin_deformer.vertices[]/weights[]`
  // instead! NOTE: These may be out-of-bounds for a given mesh,
  // `ufbx_skin_deformer.vertices` is always safe.
...
// Blend shape deformer can contain multiple channels (think of sliders between
// morphs) that may optionally have in-between keyframes.
...
UFBX_ENUM_TYPE(ufbx_cache_file_format, UFBX_CACHE_FILE_FORMAT,
...
UFBX_ENUM_TYPE(ufbx_cache_data_format, UFBX_CACHE_DATA_FORMAT,
...
typedef enum ufbx_cache_interpretation UFBX_ENUM_REPR {
  // Unknown interpretation, see `ufbx_cache_channel.interpretation_name` for
  // more information.
...
UFBX_ENUM_TYPE(ufbx_cache_interpretation, UFBX_CACHE_INTERPRETATION,
               UFBX_CACHE_INTERPRETATION_VERTEX_NORMAL);
...
typedef struct ufbx_material_map {
  ...
  // May be specified simultaneously with a texture, in this case most shading
  // models use multiplicative tinting of the texture values.
  union {
    ufbx_real value_real;
    ufbx_vec2 value_vec2;
    ufbx_vec3 value_vec3;
  ...
  // May be valid but "disabled" (application specific) if `texture_enabled ==
  // false`.
  ufbx_nullable ufbx_texture *texture;

  // `true` if the file has specified any of the values above.
  // NOTE: The value may be set to a non-zero default even if `has_value ==
  // false`, for example missing factors are set to `1.0` if a color is defined.
  bool has_value;

  // Controls whether shading should use `texture`.
  // NOTE: Some shading models allow this to be `true` even if `texture ==
  // NULL`.
...
typedef struct ufbx_material_feature_info {
  ...
  // NOTE: The feature can be enabled but still not used if eg. the
  // corresponding factor is at zero!
  bool enabled;
...
UFBX_ENUM_TYPE(ufbx_material_fbx_map, UFBX_MATERIAL_FBX_MAP,
               UFBX_MATERIAL_FBX_VECTOR_DISPLACEMENT);
...
UFBX_ENUM_TYPE(ufbx_material_pbr_map, UFBX_MATERIAL_PBR_MAP,
               UFBX_MATERIAL_PBR_TRANSMISSION_GLOSSINESS);
...
struct ufbx_material {
  ...
  ufbx_string
      shading_model_name; // < Often one of `{ "lambert", "phong", "unknown" }`

  // Prefix before shader property names with trailing `|`.
  // For example `"3dsMax|Parameters|"` where properties would have names like
  // `"3dsMax|Parameters|base_color"`. You can ignore this if you use the
  // built-in `ufbx_material_fbx_maps fbx` and `ufbx_material_pbr_maps pbr`
  // structures.
  ufbx_string shader_prop_prefix;

  // All textures attached to the material, if you want specific maps if might
  // be more convenient to use eg. `fbx.diffuse_color.texture` or
  // `pbr.base_color.texture`
...
// See eg.
// https://www.w3.org/TR/2013/WD-compositing-1-20131010/#blendingseparable
...
typedef struct ufbx_texture_layer {
  ...
  ufbx_blend_mode
      blend_mode;  // < Equation to combine the layer to the background
...
typedef struct ufbx_shader_texture {
  ...
  // Only specified if `main_texture.outputs[main_texture_output_index]` is
  // semantically equivalent to this texture.
...
struct ufbx_texture {
  ...
  // NOTE: May be specified even if `type == UFBX_TEXTURE_FILE` if
  // `ufbx_load_opts.disable_quirks` is _not_ specified. Some known shaders that
  // represent files are interpreted as `UFBX_TEXTURE_FILE`.
  ufbx_nullable ufbx_shader_texture *shader;

  // List of file textures representing this texture.
  // Defined even if `type == UFBX_TEXTURE_FILE` in which case the array
  ...
  // UV transform
  bool has_uv_transform;       // < Has a non-identity `transform` and derived
                               // matrices.
  ...
  ufbx_matrix
      uv_to_texture; // < UV coordinate to normalized texture coordinate matrix
};
...
struct ufbx_shader {
  ...
  // HINT: `ufbx_find_shader_prop()` translates shader properties to FBX
  // properties
...
// Usually obtained from `ufbx_scene` via either global animation
// `ufbx_scene.anim`, per-stack animation `ufbx_anim_stack.anim` or per-layer
// animation `ufbx_anim_layer.anim`.
//
// For advanced usage you can use `ufbx_create_anim()` to create animation
// descriptors with custom layers, property overrides, special flags, etc.
...
UFBX_ENUM_TYPE(ufbx_interpolation, UFBX_INTERPOLATION,
...
// Single real `value` at a specified `time`, interpolation between two
// keyframes is determined by the `interpolation` field of the _previous_ key.
...
// Selection state of a node, potentially contains vertex/edge/face selection as
// well.
...
typedef struct ufbx_constraint_target {
  ...
  ufbx_real
      weight; // < Relative weight to other targets (does not always sum to 1)
...
typedef enum ufbx_constraint_aim_up_type UFBX_ENUM_REPR {
  UFBX_CONSTRAINT_AIM_UP_SCENE, // < Align the up vector to the scene global up
                                // vector
  UFBX_CONSTRAINT_AIM_UP_TO_NODE,    // < Aim the up vector at
                                     // `ufbx_constraint.aim_up_node`
  UFBX_CONSTRAINT_AIM_UP_ALIGN_NODE, // < Copy the up vector from
                                     // `ufbx_constraint.aim_up_node`
  UFBX_CONSTRAINT_AIM_UP_VECTOR, // < Use `ufbx_constraint.aim_up_vector` as the
                                 // up vector
...
UFBX_ENUM_TYPE(ufbx_constraint_aim_up_type, UFBX_CONSTRAINT_AIM_UP_TYPE,
               UFBX_CONSTRAINT_AIM_UP_NONE);

// Method to determine the up vector in aim constraints
typedef enum ufbx_constraint_ik_pole_type UFBX_ENUM_REPR {
  UFBX_CONSTRAINT_IK_POLE_VECTOR, // < Use towards calculated from
                                  // `ufbx_constraint.targets`
  UFBX_CONSTRAINT_IK_POLE_NODE,   // < Use `ufbx_constraint.ik_pole_vector`
                                  // directly

  UFBX_ENUM_FORCE_WIDTH(UFBX_CONSTRAINT_IK_POLE_TYPE)
} ufbx_constraint_ik_pole_type;

UFBX_ENUM_TYPE(ufbx_constraint_ik_pole_type, UFBX_CONSTRAINT_IK_POLE_TYPE,
...
UFBX_ENUM_TYPE(ufbx_warning_type, UFBX_WARNING_TYPE,
               UFBX_WARNING_UNKNOWN_OBJ_DIRECTIVE);
...
typedef struct ufbx_warning {
  ...
  // The element related to this warning or `UFBX_NO_INDEX` if not related to a
  // specific element.
  uint32_t element_id;
...
UFBX_ENUM_TYPE(ufbx_thumbnail_format, UFBX_THUMBNAIL_FORMAT,
               UFBX_THUMBNAIL_FORMAT_RGBA_32);

// Specify how unit / coordinate system conversion should be performed.
// Affects how `ufbx_load_opts.target_axes` and
// `ufbx_load_opts.target_unit_meters` work, has no effect if neither is
// specified.
typedef enum ufbx_space_conversion UFBX_ENUM_REPR {
  ...
  // Perform the conversion by scaling geometry in addition to adjusting
  // transforms.
...
UFBX_ENUM_TYPE(ufbx_space_conversion, UFBX_SPACE_CONVERSION,
               UFBX_SPACE_CONVERSION_MODIFY_GEOMETRY);
...
UFBX_ENUM_TYPE(ufbx_time_protocol, UFBX_TIME_PROTOCOL,
               UFBX_TIME_PROTOCOL_DEFAULT);
...
typedef struct ufbx_scene_settings {
  ...
  // NOTE: This contains the _original_ axes even if you supply
  // `ufbx_load_opts.target_axes`.
...
typedef struct ufbx_topo_edge {
  ...
  uint32_t next; // < Ending index of the edge / next per-face `ufbx_topo_edge`,
                 // always defined
  uint32_t prev; // < Previous per-face `ufbx_topo_edge`, always defined
  uint32_t twin; // < `ufbx_topo_edge` on the opposite side, `UFBX_NO_INDEX` if
                 // not found
...
typedef void *ufbx_realloc_fn(void *user, void *old_ptr, size_t old_size,
                              size_t new_size);
...
UFBX_ENUM_TYPE(ufbx_open_file_type, UFBX_OPEN_FILE_TYPE,
...
typedef bool ufbx_open_file_fn(void *user, ufbx_stream *stream,
...
typedef struct ufbx_open_file_cb {
  ...
  UFBX_CALLBACK_IMPL(ufbx_open_file_cb, ufbx_open_file_fn, bool,
                     (void *user, ufbx_stream *stream, const char *path,
                      size_t path_len, const ufbx_open_file_info *info),
...
typedef struct ufbx_open_memory_opts {
  ...
  // the memory stream, make sure the data stays valid until the stream is
  // closed!
...
typedef enum ufbx_error_type UFBX_ENUM_REPR {
  ...
  // IO read error.
  // eg. returning `SIZE_MAX` from `ufbx_stream.read_fn` or stdio `ferror()`
  // condition.
  UFBX_ERROR_IO,
  ...
  // HINT: You can supply it manually using `ufbx_load_opts.file_format` or use
  // `ufbx_load_opts.filename`
  // when using `ufbx_load_memory()` to let ufbx guess the format from the
  // extension.
  ...
  // Invalid UTF-8 encountered in a file when loading with
  ...
  // Out of bounds index in the file when loading with
  ...
  // Threaded ASCII parsing is slightly more strict than non-threaded, for
  // cursed files,
...
typedef ufbx_progress_result ufbx_progress_fn(void *user,
...
typedef struct ufbx_progress_cb {
  ...
  UFBX_CALLBACK_IMPL(ufbx_progress_cb, ufbx_progress_fn, ufbx_progress_result,
                     (void *user, const ufbx_progress *progress), (progress))
} ufbx_progress_cb;
...
typedef enum ufbx_index_error_handling UFBX_ENUM_REPR {
  ...
  // This is the recommended way if you need to deal with files with gaps in
  // information.
  ...
  // UNSAFE: Breaks any API guarantees regarding indexes being in bounds and
  // makes
...
UFBX_ENUM_TYPE(ufbx_index_error_handling, UFBX_INDEX_ERROR_HANDLING,
               UFBX_INDEX_ERROR_HANDLING_UNSAFE_IGNORE);
...
UFBX_ENUM_TYPE(ufbx_unicode_error_handling, UFBX_UNICODE_ERROR_HANDLING,
               UFBX_UNICODE_ERROR_HANDLING_UNSAFE_IGNORE);

// How to handle FBX node geometry transforms.
// FBX nodes can have "geometry transforms" that affect only the attached
// meshes, but not the children. This is not allowed in many scene
// representations so ufbx provides some ways to simplify them. Geometry
// transforms can also be used to transform any other attributes such as lights
// or cameras.
typedef enum ufbx_geometry_transform_handling UFBX_ENUM_REPR {
  ...
  // `ufbx_node.geometry_to_node`, or `ufbx_node.geometry_to_world` to
  // compensate
  ...
  // if
  // necessary, for example if there are multiple instances of the same mesh
  // with
  // geometry transforms.
...
UFBX_ENUM_TYPE(ufbx_geometry_transform_handling,
               UFBX_GEOMETRY_TRANSFORM_HANDLING,
               UFBX_GEOMETRY_TRANSFORM_HANDLING_MODIFY_GEOMETRY_NO_FALLBACK);
...
UFBX_ENUM_TYPE(ufbx_inherit_mode_handling, UFBX_INHERIT_MODE_HANDLING,
               UFBX_INHERIT_MODE_HANDLING_IGNORE);
...
UFBX_ENUM_TYPE(ufbx_pivot_handling, UFBX_PIVOT_HANDLING,
               UFBX_PIVOT_HANDLING_ADJUST_TO_PIVOT);
...
typedef bool ufbx_thread_pool_init_fn(void *user, ufbx_thread_pool_context ctx,
                                      const ufbx_thread_pool_info *info);
typedef bool ufbx_thread_pool_run_fn(void *user, ufbx_thread_pool_context ctx,
                                     uint32_t group, uint32_t start_index,
                                     uint32_t count);
typedef bool ufbx_thread_pool_wait_fn(void *user, ufbx_thread_pool_context ctx,
                                      uint32_t group, uint32_t max_index);
...
typedef struct ufbx_load_opts {
  ...
  bool ignore_all_content; // < Do not load any content (geometry, animation,
                           // embedded)

  bool
      evaluate_skinning; // < Evaluate skinning (see ufbx_mesh.skinned_vertices)
  bool evaluate_caches;  // < Evaluate vertex caches (see
                         // ufbx_mesh.skinned_vertices)
  ...
  // `ufbx_load_opts.ignore_missing_external_files` to suppress this, in this
  // case you can find the errors at `ufbx_metadata.warnings[]` as
  // `UFBX_WARNING_MISSING_EXTERNAL_FILE`.
  ...
  // Skip computing `ufbx_mesh.material_parts[]` and
  // `ufbx_mesh.face_group_parts[]`.
  ...
  // You can see if the normals have been generated from
  // `ufbx_mesh.generated_normals`.
  ...
  // Will fail with `UFBX_ERROR_NODE_DEPTH_LIMIT` if a node is deeper than this
  // limit. NOTE: The default of 0 allows arbitrarily deep hierarchies. Be
  // careful if using recursive algorithms without setting this limit.
  ...
  // Axis used to mirror for conversion between left-handed and right-handed
  // coordinates.
  ...
  // meters. By default units are not scaled.
  ...
  // Animation keyframe clamp threhsold, only applies to specific interpolation
  // modes.
  ...
  // Used if the file specified `mtllib` line is not found, eg. for a file
  // called `model.obj` that contains the line `usemtl materials.mtl`, ufbx
  // would first try to open `materials.mtl` and if that fails it tries to open
  // `model.mtl`.
...
typedef struct ufbx_evaluate_opts {
  ...
  // WARNING: Potentially unsafe! Try to open external files such as geometry
  // caches
  bool load_external_files;
...
UFBX_LIST_TYPE(ufbx_const_prop_override_desc_list,
               const ufbx_prop_override_desc);

UFBX_LIST_TYPE(ufbx_const_transform_override_list,
               const ufbx_transform_override);
...
typedef struct ufbx_anim_opts {
  ...
  ufbx_allocator_opts
      result_allocator; // < Allocator used to create the `ufbx_anim`
...
typedef struct ufbx_bake_opts {
  ...
  ufbx_allocator_opts temp_allocator; // < Allocator used during loading
  ufbx_allocator_opts
      result_allocator; // < Allocator used for the final baked animation
  ...
  // Maximum sample rate to use, this will remove keys if they are too close
  // together. Default: unlimited
  ...
  // FBX interpolates rotation in Euler angles, so this might cause incorrect
  // interpolation.
  ...
  // Assumes rotations will be interpolated using a spherical linear
  ...
  // Compensate for `UFBX_INHERIT_MODE_IGNORE_PARENT_SCALE` by adjusting child
  // scale. NOTE: This is an lossy operation, and properly works only for
  // uniform scaling.
...
typedef struct ufbx_tessellate_curve_opts {
  ...
  ufbx_allocator_opts
      result_allocator; // < Allocator used for the final line curve
...
typedef struct ufbx_tessellate_surface_opts {
  ...
  // How many segments tessellate each span in `ufbx_nurbs_basis.spans`.
  // NOTE: Default is `4`, _not_ `ufbx_nurbs_surface.span_subdivision_u/v` as
  // that would make it easy to create an FBX file with an absurdly high
  // subdivision rate (similar to mesh subdivision). Please enforce copy the
  // value yourself enforcing whatever limits you deem reasonable.
  uint32_t span_subdivision_u;
  uint32_t span_subdivision_v;
...
// Practically always `true` (see below), if not you need to be careful with
// threads.
//
...
ufbx_abi ufbx_scene *ufbx_load_memory(const void *data, size_t data_size,
                                      const ufbx_load_opts *opts,
                                      ufbx_error *error);

// Load a scene by opening a file named `filename`
ufbx_abi ufbx_scene *ufbx_load_file(const char *filename,
                                    const ufbx_load_opts *opts,
                                    ufbx_error *error);
ufbx_abi ufbx_scene *ufbx_load_file_len(const char *filename,
                                        size_t filename_len,
                                        const ufbx_load_opts *opts,
                                        ufbx_error *error);
...
ufbx_abi ufbx_scene *ufbx_load_stdio(void *file, const ufbx_load_opts *opts,
                                     ufbx_error *error);

// Load a scene by reading from an `FILE *file` stream with a prefix
// NOTE: `file` is passed as a `void` pointer to avoid including <stdio.h>
ufbx_abi ufbx_scene *ufbx_load_stdio_prefix(void *file, const void *prefix,
                                            size_t prefix_size,
                                            const ufbx_load_opts *opts,
                                            ufbx_error *error);

// Load a scene from a user-specified stream
ufbx_abi ufbx_scene *ufbx_load_stream(const ufbx_stream *stream,
                                      const ufbx_load_opts *opts,
                                      ufbx_error *error);

// Load a scene from a user-specified stream with a prefix
ufbx_abi ufbx_scene *ufbx_load_stream_prefix(const ufbx_stream *stream,
                                             const void *prefix,
                                             size_t prefix_size,
                                             const ufbx_load_opts *opts,
                                             ufbx_error *error);
...
// Always produces a NULL-terminated string to `char dst[dst_size]`, truncating
// if necessary. Returns the number of characters written not including the NULL
// terminator.
ufbx_abi size_t ufbx_format_error(char *dst, size_t dst_size,
...
ufbx_abi ufbx_prop *ufbx_find_prop_len(const ufbx_props *props,
                                       const char *name, size_t name_len);
ufbx_inline ufbx_prop *ufbx_find_prop(const ufbx_props *props,
                                      const char *name) {
  return ufbx_find_prop_len(props, name, strlen(name));
}

// Utility functions for finding the value of a property, returns `def` if not
// found. NOTE: For `ufbx_string` you need to ensure the lifetime of the default
// is sufficient as no copy is made.
ufbx_abi ufbx_real ufbx_find_real_len(const ufbx_props *props, const char *name,
                                      size_t name_len, ufbx_real def);
ufbx_inline ufbx_real ufbx_find_real(const ufbx_props *props, const char *name,
                                     ufbx_real def) {
  return ufbx_find_real_len(props, name, strlen(name), def);
}
ufbx_abi ufbx_vec3 ufbx_find_vec3_len(const ufbx_props *props, const char *name,
                                      size_t name_len, ufbx_vec3 def);
ufbx_inline ufbx_vec3 ufbx_find_vec3(const ufbx_props *props, const char *name,
                                     ufbx_vec3 def) {
  return ufbx_find_vec3_len(props, name, strlen(name), def);
}
ufbx_abi int64_t ufbx_find_int_len(const ufbx_props *props, const char *name,
                                   size_t name_len, int64_t def);
ufbx_inline int64_t ufbx_find_int(const ufbx_props *props, const char *name,
                                  int64_t def) {
  return ufbx_find_int_len(props, name, strlen(name), def);
}
ufbx_abi bool ufbx_find_bool_len(const ufbx_props *props, const char *name,
                                 size_t name_len, bool def);
ufbx_inline bool ufbx_find_bool(const ufbx_props *props, const char *name,
                                bool def) {
  return ufbx_find_bool_len(props, name, strlen(name), def);
}
ufbx_abi ufbx_string ufbx_find_string_len(const ufbx_props *props,
                                          const char *name, size_t name_len,
                                          ufbx_string def);
ufbx_inline ufbx_string ufbx_find_string(const ufbx_props *props,
                                         const char *name, ufbx_string def) {
  return ufbx_find_string_len(props, name, strlen(name), def);
}
ufbx_abi ufbx_blob ufbx_find_blob_len(const ufbx_props *props, const char *name,
                                      size_t name_len, ufbx_blob def);
ufbx_inline ufbx_blob ufbx_find_blob(const ufbx_props *props, const char *name,
                                     ufbx_blob def) {
  return ufbx_find_blob_len(props, name, strlen(name), def);
}

// Find property in `props` with concatendated `parts[num_parts]`.
ufbx_abi ufbx_prop *ufbx_find_prop_concat(const ufbx_props *props,
                                          const ufbx_string *parts,
                                          size_t num_parts);

// Get an element connected to a property.
ufbx_abi ufbx_element *ufbx_get_prop_element(const ufbx_element *element,
                                             const ufbx_prop *prop,
                                             ufbx_element_type type);

// Find an element connected to a property by name.
ufbx_abi ufbx_element *ufbx_find_prop_element_len(const ufbx_element *element,
                                                  const char *name,
                                                  size_t name_len,
                                                  ufbx_element_type type);
ufbx_inline ufbx_element *ufbx_find_prop_element(const ufbx_element *element,
                                                 const char *name,
                                                 ufbx_element_type type) {
...
ufbx_abi ufbx_element *ufbx_find_element_len(const ufbx_scene *scene,
                                             ufbx_element_type type,
                                             const char *name, size_t name_len);
ufbx_inline ufbx_element *ufbx_find_element(const ufbx_scene *scene,
                                            ufbx_element_type type,
                                            const char *name) {
  return ufbx_find_element_len(scene, type, name, strlen(name));
}

// Find node in `scene` by `name` (shorthand for
// `ufbx_find_element(UFBX_ELEMENT_NODE)`).
ufbx_abi ufbx_node *ufbx_find_node_len(const ufbx_scene *scene,
                                       const char *name, size_t name_len);
ufbx_inline ufbx_node *ufbx_find_node(const ufbx_scene *scene,
                                      const char *name) {
  return ufbx_find_node_len(scene, name, strlen(name));
}

// Find an animation stack in `scene` by `name` (shorthand for
// `ufbx_find_element(UFBX_ELEMENT_ANIM_STACK)`)
ufbx_abi ufbx_anim_stack *ufbx_find_anim_stack_len(const ufbx_scene *scene,
                                                   const char *name,
                                                   size_t name_len);
ufbx_inline ufbx_anim_stack *ufbx_find_anim_stack(const ufbx_scene *scene,
                                                  const char *name) {
  return ufbx_find_anim_stack_len(scene, name, strlen(name));
}

// Find a material in `scene` by `name` (shorthand for
// `ufbx_find_element(UFBX_ELEMENT_MATERIAL)`).
ufbx_abi ufbx_material *ufbx_find_material_len(const ufbx_scene *scene,
                                               const char *name,
                                               size_t name_len);
ufbx_inline ufbx_material *ufbx_find_material(const ufbx_scene *scene,
                                              const char *name) {
...
ufbx_abi ufbx_anim_prop *ufbx_find_anim_prop_len(const ufbx_anim_layer *layer,
                                                 const ufbx_element *element,
                                                 const char *prop,
                                                 size_t prop_len);
ufbx_inline ufbx_anim_prop *ufbx_find_anim_prop(const ufbx_anim_layer *layer,
                                                const ufbx_element *element,
                                                const char *prop) {
...
ufbx_abi ufbx_anim_prop_list ufbx_find_anim_props(const ufbx_anim_layer *layer,
                                                  const ufbx_element *element);

// Get a matrix that transforms normals in the same way as Autodesk software.
// NOTE: The resulting normals are slightly incorrect as this function
// deliberately inverts geometric transformation wrong. For better results use
// `ufbx_matrix_for_normals(&node->geometry_to_world)`.
ufbx_abi ufbx_matrix
ufbx_get_compatible_matrix_for_normals(const ufbx_node *node);

// Utility

// Decompress a DEFLATE compressed buffer.
// Returns the decompressed size or a negative error code (see source for
// details). NOTE: You must supply a valid `retain` with
// `ufbx_inflate_retain.initialized == false` but the rest can be uninitialized.
ufbx_abi ptrdiff_t ufbx_inflate(void *dst, size_t dst_size,
                                const ufbx_inflate_input *input,
                                ufbx_inflate_retain *retain);

// Open a `ufbx_stream` from a file.
// Use `path_len == SIZE_MAX` for NULL terminated string.
ufbx_abi bool ufbx_open_file(ufbx_stream *stream, const char *path,
                             size_t path_len);

// Same as `ufbx_open_file()` but compatible with the callback in
// `ufbx_open_file_fn`. The `user` parameter is actually not used here.
ufbx_abi bool ufbx_default_open_file(void *user, ufbx_stream *stream,
                                     const char *path, size_t path_len,
                                     const ufbx_open_file_info *info);

// NOTE: Uses the default ufbx allocator!
ufbx_abi bool ufbx_open_memory(ufbx_stream *stream, const void *data,
                               size_t data_size,
                               const ufbx_open_memory_opts *opts,
                               ufbx_error *error);
...
ufbx_abi ufbx_real ufbx_evaluate_curve(const ufbx_anim_curve *curve,
                                       double time, ufbx_real default_value);

// Evaluate a value from bundled animation curves.
ufbx_abi ufbx_real
ufbx_evaluate_anim_value_real(const ufbx_anim_value *anim_value, double time);
ufbx_abi ufbx_vec2
ufbx_evaluate_anim_value_vec2(const ufbx_anim_value *anim_value, double time);
ufbx_abi ufbx_vec3
ufbx_evaluate_anim_value_vec3(const ufbx_anim_value *anim_value, double time);

// Evaluate an animated property `name` from `element` at `time`.
// NOTE: If the property is not found it will have the flag
// `UFBX_PROP_FLAG_NOT_FOUND`.
ufbx_abi ufbx_prop ufbx_evaluate_prop_len(const ufbx_anim *anim,
                                          const ufbx_element *element,
                                          const char *name, size_t name_len,
                                          double time);
ufbx_inline ufbx_prop ufbx_evaluate_prop(const ufbx_anim *anim,
                                         const ufbx_element *element,
...
// HINT: This function returns an `ufbx_props` structure with the original
// properties as `ufbx_props.defaults`. This lets you use
// `ufbx_find_prop/value()` for the results.
ufbx_abi ufbx_props ufbx_evaluate_props(const ufbx_anim *anim,
                                        const ufbx_element *element,
                                        double time, ufbx_prop *buffer,
                                        size_t buffer_size);
...
ufbx_abi ufbx_transform ufbx_evaluate_transform(const ufbx_anim *anim,
                                                const ufbx_node *node,
                                                double time);
ufbx_abi ufbx_transform ufbx_evaluate_transform_flags(const ufbx_anim *anim,
                                                      const ufbx_node *node,
                                                      double time,
                                                      uint32_t flags);

ufbx_abi ufbx_real ufbx_evaluate_blend_weight(const ufbx_anim *anim,
...
ufbx_abi ufbx_scene *ufbx_evaluate_scene(const ufbx_scene *scene,
                                         const ufbx_anim *anim, double time,
                                         const ufbx_evaluate_opts *opts,
                                         ufbx_error *error);

ufbx_abi ufbx_anim *ufbx_create_anim(const ufbx_scene *scene,
                                     const ufbx_anim_opts *opts,
                                     ufbx_error *error);
...
ufbx_abi ufbx_baked_anim *ufbx_bake_anim(const ufbx_scene *scene,
                                         const ufbx_anim *anim,
                                         const ufbx_bake_opts *opts,
                                         ufbx_error *error);

ufbx_abi void ufbx_retain_baked_anim(ufbx_baked_anim *bake);
ufbx_abi void ufbx_free_baked_anim(ufbx_baked_anim *bake);

ufbx_abi ufbx_vec3 ufbx_evaluate_baked_vec3(ufbx_baked_vec3_list keyframes,
                                            double time);
ufbx_abi ufbx_quat ufbx_evaluate_baked_quat(ufbx_baked_quat_list keyframes,
                                            double time);

// Poses

ufbx_abi ufbx_bone_pose *ufbx_get_bone_pose(const ufbx_pose *pose,
                                            const ufbx_node *node);

// Materials

ufbx_abi ufbx_texture *ufbx_find_prop_texture_len(const ufbx_material *material,
                                                  const char *name,
                                                  size_t name_len);
ufbx_inline ufbx_texture *ufbx_find_prop_texture(const ufbx_material *material,
                                                 const char *name) {
  return ufbx_find_prop_texture_len(material, name, strlen(name));
}

ufbx_abi ufbx_string ufbx_find_shader_prop_len(const ufbx_shader *shader,
                                               const char *name,
                                               size_t name_len);
ufbx_inline ufbx_string ufbx_find_shader_prop(const ufbx_shader *shader,
                                              const char *name) {
  return ufbx_find_shader_prop_len(shader, name, strlen(name));
}

ufbx_abi ufbx_shader_prop_binding_list ufbx_find_shader_prop_bindings_len(
    const ufbx_shader *shader, const char *name, size_t name_len);
ufbx_inline ufbx_shader_prop_binding_list
ufbx_find_shader_prop_bindings(const ufbx_shader *shader, const char *name) {
  return ufbx_find_shader_prop_bindings_len(shader, name, strlen(name));
}

ufbx_abi ufbx_shader_texture_input *
ufbx_find_shader_texture_input_len(const ufbx_shader_texture *shader,
                                   const char *name, size_t name_len);
ufbx_inline ufbx_shader_texture_input *
ufbx_find_shader_texture_input(const ufbx_shader_texture *shader,
                               const char *name) {
...
ufbx_abi ufbx_matrix ufbx_matrix_mul(const ufbx_matrix *a,
                                     const ufbx_matrix *b);
...
ufbx_abi ufbx_matrix ufbx_catch_get_skin_vertex_matrix(
    ufbx_panic *panic, const ufbx_skin_deformer *skin, size_t vertex,
    const ufbx_matrix *fallback);
ufbx_inline ufbx_matrix
ufbx_get_skin_vertex_matrix(const ufbx_skin_deformer *skin, size_t vertex,
                            const ufbx_matrix *fallback) {
  return ufbx_catch_get_skin_vertex_matrix(NULL, skin, vertex, fallback);
}

ufbx_abi uint32_t
ufbx_get_blend_shape_offset_index(const ufbx_blend_shape *shape, size_t vertex);
ufbx_abi ufbx_vec3 ufbx_get_blend_shape_vertex_offset(
    const ufbx_blend_shape *shape, size_t vertex);
ufbx_abi ufbx_vec3
ufbx_get_blend_vertex_offset(const ufbx_blend_deformer *blend, size_t vertex);

ufbx_abi void ufbx_add_blend_shape_vertex_offsets(const ufbx_blend_shape *shape,
                                                  ufbx_vec3 *vertices,
                                                  size_t num_vertices,
                                                  ufbx_real weight);
ufbx_abi void ufbx_add_blend_vertex_offsets(const ufbx_blend_deformer *blend,
                                            ufbx_vec3 *vertices,
                                            size_t num_vertices,
                                            ufbx_real weight);
...
ufbx_abi size_t ufbx_evaluate_nurbs_basis(const ufbx_nurbs_basis *basis,
                                          ufbx_real u, ufbx_real *weights,
                                          size_t num_weights,
                                          ufbx_real *derivatives,
                                          size_t num_derivatives);

ufbx_abi ufbx_curve_point
ufbx_evaluate_nurbs_curve(const ufbx_nurbs_curve *curve, ufbx_real u);
ufbx_abi ufbx_surface_point ufbx_evaluate_nurbs_surface(
    const ufbx_nurbs_surface *surface, ufbx_real u, ufbx_real v);

ufbx_abi ufbx_line_curve *
ufbx_tessellate_nurbs_curve(const ufbx_nurbs_curve *curve,
                            const ufbx_tessellate_curve_opts *opts,
                            ufbx_error *error);
ufbx_abi ufbx_mesh *
ufbx_tessellate_nurbs_surface(const ufbx_nurbs_surface *surface,
                              const ufbx_tessellate_surface_opts *opts,
                              ufbx_error *error);
...
ufbx_abi uint32_t ufbx_catch_triangulate_face(ufbx_panic *panic,
                                              uint32_t *indices,
                                              size_t num_indices,
                                              const ufbx_mesh *mesh,
                                              ufbx_face face);
ufbx_inline uint32_t ufbx_triangulate_face(uint32_t *indices,
                                           size_t num_indices,
                                           const ufbx_mesh *mesh,
                                           ufbx_face face) {
...
ufbx_abi void ufbx_catch_compute_topology(ufbx_panic *panic,
                                          const ufbx_mesh *mesh,
                                          ufbx_topo_edge *topo,
                                          size_t num_topo);
ufbx_inline void ufbx_compute_topology(const ufbx_mesh *mesh,
                                       ufbx_topo_edge *topo, size_t num_topo) {
  ufbx_catch_compute_topology(NULL, mesh, topo, num_topo);
}

// Get the next/previous edge around a vertex
// NOTE: Does not return the half-edge on the opposite side (ie.
// `topo[index].twin`)

ufbx_abi uint32_t ufbx_catch_topo_next_vertex_edge(ufbx_panic *panic,
                                                   const ufbx_topo_edge *topo,
                                                   size_t num_topo,
                                                   uint32_t index);
ufbx_inline uint32_t ufbx_topo_next_vertex_edge(const ufbx_topo_edge *topo,
                                                size_t num_topo,
                                                uint32_t index) {
  return ufbx_catch_topo_next_vertex_edge(NULL, topo, num_topo, index);
}

ufbx_abi uint32_t ufbx_catch_topo_prev_vertex_edge(ufbx_panic *panic,
                                                   const ufbx_topo_edge *topo,
                                                   size_t num_topo,
                                                   uint32_t index);
ufbx_inline uint32_t ufbx_topo_prev_vertex_edge(const ufbx_topo_edge *topo,
                                                size_t num_topo,
                                                uint32_t index) {
  return ufbx_catch_topo_prev_vertex_edge(NULL, topo, num_topo, index);
}

ufbx_abi ufbx_vec3 ufbx_catch_get_weighted_face_normal(
    ufbx_panic *panic, const ufbx_vertex_vec3 *positions, ufbx_face face);
ufbx_inline ufbx_vec3 ufbx_get_weighted_face_normal(
    const ufbx_vertex_vec3 *positions, ufbx_face face) {
  return ufbx_catch_get_weighted_face_normal(NULL, positions, face);
}

ufbx_abi size_t ufbx_catch_generate_normal_mapping(
    ufbx_panic *panic, const ufbx_mesh *mesh, const ufbx_topo_edge *topo,
    size_t num_topo, uint32_t *normal_indices, size_t num_normal_indices,
    bool assume_smooth);
ufbx_abi size_t ufbx_generate_normal_mapping(
    const ufbx_mesh *mesh, const ufbx_topo_edge *topo, size_t num_topo,
    uint32_t *normal_indices, size_t num_normal_indices, bool assume_smooth);

ufbx_abi void ufbx_catch_compute_normals(
    ufbx_panic *panic, const ufbx_mesh *mesh, const ufbx_vertex_vec3 *positions,
    const uint32_t *normal_indices, size_t num_normal_indices,
    ufbx_vec3 *normals, size_t num_normals);
ufbx_abi void ufbx_compute_normals(const ufbx_mesh *mesh,
                                   const ufbx_vertex_vec3 *positions,
                                   const uint32_t *normal_indices,
                                   size_t num_normal_indices,
                                   ufbx_vec3 *normals, size_t num_normals);

ufbx_abi ufbx_mesh *ufbx_subdivide_mesh(const ufbx_mesh *mesh, size_t level,
                                        const ufbx_subdivide_opts *opts,
                                        ufbx_error *error);
...
ufbx_abi ufbx_geometry_cache *
ufbx_load_geometry_cache(const char *filename,
                         const ufbx_geometry_cache_opts *opts,
                         ufbx_error *error);
ufbx_abi ufbx_geometry_cache *
ufbx_load_geometry_cache_len(const char *filename, size_t filename_len,
                             const ufbx_geometry_cache_opts *opts,
                             ufbx_error *error);
...
ufbx_abi size_t ufbx_read_geometry_cache_real(
    const ufbx_cache_frame *frame, ufbx_real *data, size_t num_data,
    const ufbx_geometry_cache_data_opts *opts);
ufbx_abi size_t ufbx_sample_geometry_cache_real(
    const ufbx_cache_channel *channel, double time, ufbx_real *data,
    size_t num_data, const ufbx_geometry_cache_data_opts *opts);
ufbx_abi size_t ufbx_read_geometry_cache_vec3(
    const ufbx_cache_frame *frame, ufbx_vec3 *data, size_t num_data,
    const ufbx_geometry_cache_data_opts *opts);
ufbx_abi size_t ufbx_sample_geometry_cache_vec3(
    const ufbx_cache_channel *channel, double time, ufbx_vec3 *data,
    size_t num_data, const ufbx_geometry_cache_data_opts *opts);

// DOM

ufbx_abi ufbx_dom_node *ufbx_dom_find_len(const ufbx_dom_node *parent,
                                          const char *name, size_t name_len);
ufbx_inline ufbx_dom_node *ufbx_dom_find(const ufbx_dom_node *parent,
                                         const char *name) {
...
ufbx_abi size_t ufbx_generate_indices(const ufbx_vertex_stream *streams,
                                      size_t num_streams, uint32_t *indices,
                                      size_t num_indices,
                                      const ufbx_allocator_opts *allocator,
                                      ufbx_error *error);
...
ufbx_unsafe ufbx_abi void
ufbx_thread_pool_run_task(ufbx_thread_pool_context ctx, uint32_t index);

ufbx_unsafe ufbx_abi void
ufbx_thread_pool_set_user_ptr(ufbx_thread_pool_context ctx, void *user_ptr);
ufbx_unsafe ufbx_abi void *
ufbx_thread_pool_get_user_ptr(ufbx_thread_pool_context ctx);

// -- Inline API

ufbx_abi ufbx_real ufbx_catch_get_vertex_real(ufbx_panic *panic,
                                              const ufbx_vertex_real *v,
                                              size_t index);
ufbx_abi ufbx_vec2 ufbx_catch_get_vertex_vec2(ufbx_panic *panic,
                                              const ufbx_vertex_vec2 *v,
                                              size_t index);
ufbx_abi ufbx_vec3 ufbx_catch_get_vertex_vec3(ufbx_panic *panic,
                                              const ufbx_vertex_vec3 *v,
                                              size_t index);
ufbx_abi ufbx_vec4 ufbx_catch_get_vertex_vec4(ufbx_panic *panic,
                                              const ufbx_vertex_vec4 *v,
                                              size_t index);

ufbx_inline ufbx_real ufbx_get_vertex_real(const ufbx_vertex_real *v,
                                           size_t index) {
  ufbx_assert(index < v->indices.count);
  return v->values.data[(int32_t)v->indices.data[index]];
}
ufbx_inline ufbx_vec2 ufbx_get_vertex_vec2(const ufbx_vertex_vec2 *v,
                                           size_t index) {
  ufbx_assert(index < v->indices.count);
  return v->values.data[(int32_t)v->indices.data[index]];
}
ufbx_inline ufbx_vec3 ufbx_get_vertex_vec3(const ufbx_vertex_vec3 *v,
                                           size_t index) {
  ufbx_assert(index < v->indices.count);
  return v->values.data[(int32_t)v->indices.data[index]];
}
ufbx_inline ufbx_vec4 ufbx_get_vertex_vec4(const ufbx_vertex_vec4 *v,
                                           size_t index) {
  ufbx_assert(index < v->indices.count);
  return v->values.data[(int32_t)v->indices.data[index]];
}
...
ufbx_abi ufbx_nurbs_trim_surface *
ufbx_as_nurbs_trim_surface(const ufbx_element *element);
ufbx_abi ufbx_nurbs_trim_boundary *
ufbx_as_nurbs_trim_boundary(const ufbx_element *element);
ufbx_abi ufbx_procedural_geometry *
ufbx_as_procedural_geometry(const ufbx_element *element);
ufbx_abi ufbx_stereo_camera *ufbx_as_stereo_camera(const ufbx_element *element);
ufbx_abi ufbx_camera_switcher *
ufbx_as_camera_switcher(const ufbx_element *element);
...
ufbx_abi ufbx_blend_deformer *
ufbx_as_blend_deformer(const ufbx_element *element);
...
ufbx_abi ufbx_cache_deformer *
ufbx_as_cache_deformer(const ufbx_element *element);
...
ufbx_abi ufbx_shader_binding *
ufbx_as_shader_binding(const ufbx_element *element);
...
ufbx_abi ufbx_selection_node *
ufbx_as_selection_node(const ufbx_element *element);
...
ufbx_abi ufbx_metadata_object *
ufbx_as_metadata_object(const ufbx_element *element);

// -- FFI API

ufbx_abi void ufbx_ffi_find_int_len(int64_t *retval, const ufbx_props *props,
                                    const char *name, size_t name_len,
                                    const int64_t *def);
ufbx_abi void ufbx_ffi_find_vec3_len(ufbx_vec3 *retval, const ufbx_props *props,
                                     const char *name, size_t name_len,
                                     const ufbx_vec3 *def);
ufbx_abi void ufbx_ffi_find_string_len(ufbx_string *retval,
                                       const ufbx_props *props,
                                       const char *name, size_t name_len,
                                       const ufbx_string *def);
ufbx_abi void ufbx_ffi_find_anim_props(ufbx_anim_prop_list *retval,
                                       const ufbx_anim_layer *layer,
                                       const ufbx_element *element);
ufbx_abi void ufbx_ffi_get_compatible_matrix_for_normals(ufbx_matrix *retval,
                                                         const ufbx_node *node);
ufbx_abi void ufbx_ffi_evaluate_anim_value_vec2(
    ufbx_vec2 *retval, const ufbx_anim_value *anim_value, double time);
ufbx_abi void ufbx_ffi_evaluate_anim_value_vec3(
    ufbx_vec3 *retval, const ufbx_anim_value *anim_value, double time);
ufbx_abi void ufbx_ffi_evaluate_prop_len(ufbx_prop *retval,
                                         const ufbx_anim *anim,
                                         const ufbx_element *element,
                                         const char *name, size_t name_len,
                                         double time);
ufbx_abi void ufbx_ffi_evaluate_props(ufbx_props *retval, const ufbx_anim *anim,
                                      ufbx_element *element, double time,
                                      ufbx_prop *buffer, size_t buffer_size);
ufbx_abi void ufbx_ffi_evaluate_transform(ufbx_transform *retval,
                                          const ufbx_anim *anim,
                                          const ufbx_node *node, double time);
ufbx_abi ufbx_real ufbx_ffi_evaluate_blend_weight(
    const ufbx_anim *anim, const ufbx_blend_channel *channel, double time);
ufbx_abi void ufbx_ffi_quat_mul(ufbx_quat *retval, const ufbx_quat *a,
                                const ufbx_quat *b);
ufbx_abi void ufbx_ffi_quat_normalize(ufbx_quat *retval, const ufbx_quat *q);
ufbx_abi void ufbx_ffi_quat_fix_antipodal(ufbx_quat *retval, const ufbx_quat *q,
                                          const ufbx_quat *reference);
ufbx_abi void ufbx_ffi_quat_slerp(ufbx_quat *retval, const ufbx_quat *a,
                                  const ufbx_quat *b, ufbx_real t);
ufbx_abi void ufbx_ffi_quat_rotate_vec3(ufbx_vec3 *retval, const ufbx_quat *q,
                                        const ufbx_vec3 *v);
ufbx_abi void ufbx_ffi_quat_to_euler(ufbx_vec3 *retval, const ufbx_quat *q,
                                     ufbx_rotation_order order);
ufbx_abi void ufbx_ffi_euler_to_quat(ufbx_quat *retval, const ufbx_vec3 *v,
                                     ufbx_rotation_order order);
ufbx_abi void ufbx_ffi_matrix_mul(ufbx_matrix *retval, const ufbx_matrix *a,
                                  const ufbx_matrix *b);
ufbx_abi void ufbx_ffi_matrix_invert(ufbx_matrix *retval, const ufbx_matrix *m);
ufbx_abi void ufbx_ffi_matrix_for_normals(ufbx_matrix *retval,
                                          const ufbx_matrix *m);
ufbx_abi void ufbx_ffi_transform_position(ufbx_vec3 *retval,
                                          const ufbx_matrix *m,
                                          const ufbx_vec3 *v);
ufbx_abi void ufbx_ffi_transform_direction(ufbx_vec3 *retval,
                                           const ufbx_matrix *m,
                                           const ufbx_vec3 *v);
ufbx_abi void ufbx_ffi_transform_to_matrix(ufbx_matrix *retval,
                                           const ufbx_transform *t);
ufbx_abi void ufbx_ffi_matrix_to_transform(ufbx_transform *retval,
                                           const ufbx_matrix *m);
ufbx_abi void ufbx_ffi_get_skin_vertex_matrix(ufbx_matrix *retval,
                                              const ufbx_skin_deformer *skin,
                                              size_t vertex,
                                              const ufbx_matrix *fallback);
ufbx_abi void ufbx_ffi_get_blend_shape_vertex_offset(
    ufbx_vec3 *retval, const ufbx_blend_shape *shape, size_t vertex);
ufbx_abi void ufbx_ffi_get_blend_vertex_offset(ufbx_vec3 *retval,
                                               const ufbx_blend_deformer *blend,
                                               size_t vertex);
ufbx_abi void ufbx_ffi_evaluate_nurbs_curve(ufbx_curve_point *retval,
                                            const ufbx_nurbs_curve *curve,
                                            ufbx_real u);
ufbx_abi void ufbx_ffi_evaluate_nurbs_surface(ufbx_surface_point *retval,
                                              const ufbx_nurbs_surface *surface,
                                              ufbx_real u, ufbx_real v);
ufbx_abi void
ufbx_ffi_get_weighted_face_normal(ufbx_vec3 *retval,
                                  const ufbx_vertex_vec3 *positions,
                                  const ufbx_face *face);
ufbx_abi size_t
ufbx_ffi_get_triangulate_face_num_indices(const ufbx_face *face);
ufbx_abi uint32_t ufbx_ffi_triangulate_face(uint32_t *indices,
                                            size_t num_indices,
                                            const ufbx_mesh *mesh,
                                            const ufbx_face *face);
ufbx_abi ufbx_vec3 ufbx_ffi_evaluate_baked_vec3(
    const ufbx_baked_vec3 *keyframes, size_t num_keyframes, double time);
ufbx_abi ufbx_quat ufbx_ffi_evaluate_baked_quat(
    const ufbx_baked_quat *keyframes, size_t num_keyframes, double time);
...
struct ufbx_string_view {
  ...
  ufbx_string_view(const char *data_, size_t length_)
      : data(data_), length(length_) {}
  UFBX_CONVERSION_TO_IMPL(ufbx_string_view)
};

ufbx_inline ufbx_scene *ufbx_load_file(ufbx_string_view filename,
                                       const ufbx_load_opts *opts,
                                       ufbx_error *error) {
  return ufbx_load_file_len(filename.data, filename.length, opts, error);
}
ufbx_inline ufbx_prop *ufbx_find_prop(const ufbx_props *props,
                                      ufbx_string_view name) {
  return ufbx_find_prop_len(props, name.data, name.length);
}
ufbx_inline ufbx_real ufbx_find_real(const ufbx_props *props,
                                     ufbx_string_view name, ufbx_real def) {
  return ufbx_find_real_len(props, name.data, name.length, def);
}
ufbx_inline ufbx_vec3 ufbx_find_vec3(const ufbx_props *props,
                                     ufbx_string_view name, ufbx_vec3 def) {
  return ufbx_find_vec3_len(props, name.data, name.length, def);
}
ufbx_inline int64_t ufbx_find_int(const ufbx_props *props,
                                  ufbx_string_view name, int64_t def) {
  return ufbx_find_int_len(props, name.data, name.length, def);
}
ufbx_inline bool ufbx_find_bool(const ufbx_props *props, ufbx_string_view name,
                                bool def) {
...
ufbx_inline ufbx_blob ufbx_find_blob(const ufbx_props *props,
                                     ufbx_string_view name, ufbx_blob def) {
...
ufbx_inline ufbx_shader_prop_binding_list ufbx_find_shader_prop_bindings(
...
ufbx_find_shader_texture_input(const ufbx_shader_texture *shader,
                               ufbx_string_view name) {
  return ufbx_find_shader_texture_input_len(shader, name.data, name.length);
}
ufbx_inline ufbx_geometry_cache *
ufbx_load_geometry_cache(ufbx_string_view filename,
                         const ufbx_geometry_cache_opts *opts,
                         ufbx_error *error) {
  return ufbx_load_geometry_cache_len(filename.data, filename.length, opts,
                                      error);
}
ufbx_inline ufbx_dom_node *ufbx_dom_find(const ufbx_dom_node *parent,
                                         ufbx_string_view name) {
...
template <typename T> struct ufbx_type_traits {
  enum { valid = 0 };
};
...
template <> struct ufbx_type_traits<ufbx_geometry_cache> {
  enum { valid = 1 };
  static void retain(ufbx_geometry_cache *ptr) {
    ufbx_retain_geometry_cache(ptr);
  }
...
public:
  template <typename T> void operator()(T *ptr) const {
    static_assert(ufbx_type_traits<T>::valid,
...
template <typename T> class ufbx_unique_ptr {
  T *ptr;
...
public:
  ...
  ufbx_unique_ptr(ufbx_unique_ptr &&ref) noexcept : ptr(ref.ptr) {
    ref.ptr = nullptr;
  }
...
// Behaves like `std::shared_ptr<T>` except uses ufbx's internal reference
// counting, so it is half the size of a standard `shared_ptr` but might be
// marginally slower.
template <typename T> class ufbx_shared_ptr {
  T *ptr;
  using traits = ufbx_type_traits<T>;
  static_assert(ufbx_type_traits<T>::valid,
...
public:
  ...
  ufbx_shared_ptr(const ufbx_shared_ptr &ref) noexcept : ptr(ref.ptr) {
    traits::retain(ref.ptr);
  }
  ufbx_shared_ptr(ufbx_shared_ptr &&ref) noexcept : ptr(ref.ptr) {
    ref.ptr = nullptr;
  }
