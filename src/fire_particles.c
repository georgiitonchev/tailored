...
static float s_update_timer = 0;
static bool s_update_particles = false;

void draw_fire_particles() {

    s_update_particles = false;
    s_update_timer += t_delta_time();

    glUseProgram(s_shader_particles);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_COLOR);

    if (s_update_timer >= 1.0f / 60) { 
        s_update_timer = 0.0f;
        s_update_particles = true;
    }
    ...
    for (unsigned int i = 0; i < s_list_particles->size; i ++) {
        ...
        if (s_update_particles && particle->life_current > 0) {
            ...
            // particle->rect.y -= ((450 * perlin_sample) - 100) * t_delta_time() * (particle->rect.height * 0.2f);

            float size = 0;
            t_ease_out_quint_f(&particle->timer, &size, particle->size_initial, 0, particle->life_initial);
